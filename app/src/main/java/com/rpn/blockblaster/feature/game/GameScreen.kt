...
fun GameScreen(
    ...
    LaunchedEffect(Unit) {
        vm.onIntent(GameIntent.StartGame)
    }
    ...
    BoxWithConstraints(
        ...
        // ── Drop-offset tuning ───────────────────────────────────────────────
        // Lower values move the ghost block CLOSER to the finger, making
        // bottom-row drops much easier. 1.0 = block top sits at finger Y.
        // 1.1 (portrait) / 0.8 (landscape) strikes a "easy but visible" balance.
        val currentOffsetFactor = if (isLandscape) 1.0f else 1.4f
        ...
        fun computeSnapCell(fx: Float, fy: Float, idx: Int): Pair<Int, Int> {
            ...
            // +0.25 vertical bias snaps one row lower → bottom row much easier to reach
            val r = (fRow + 0.25f).roundToInt().coerceIn(0, BOARD_SIZE - blk.rows)
            // Centre-align the block horizontally on the finger
            ...
            Column(
                ...
                Box(
                    ...
                    Column(
                        ...
                        // Board occupies up to 96 % of width or 62 % of height,
                        // whichever is smaller — giving the tray more room below.
                        val portraitBoardSize = minOf(boxWidth * 0.96f, boxHeight * 0.62f)
                        Box(
                            modifier = Modifier
                ...
                BlockTray(
                    ...
                    onDragEnd = { fx, fy ->
                        ...
                        if (isTrayDrop && trayHeight > 0f) {
                            // Slot-swap logic: release inside tray moves to target slot
...
// ─────────────────────────────────────────────────────────────────────────────
//  Starfield background  (richer: stars + drifting nebula clouds)
// ─────────────────────────────────────────────────────────────────────────────

@Composable
private fun StarfieldBackground() {
    // Static star positions: (x%, y%, radius, twinkle-phase-offset)
    val stars = remember {
        List(70) {
            val x     = (0..1000).random() / 1000f
            val y     = (0..1000).random() / 1000f
            val r     = (15..55).random() / 10f        // 1.5–5.5 px
            val phase = (0..628).random() / 100f       // 0–2π offset
            floatArrayOf(x, y, r, phase)
        }
    }

    // 4 drifting nebula blobs: (x%, y%, radius%, colour hue 0-1)
    val nebulae = remember {
        List(4) {
            floatArrayOf(
                (100..900).random() / 1000f,
                (100..900).random() / 1000f,
                (120..220).random() / 1000f,          // 12–22 % of screen
                (0..100).random() / 100f               // colour hue
            )
        }
    }
    ...
    // Master time counter (0 → 2π over 6 s, looping) — drives twinkle & drift
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        // ── Nebula clouds (very faint, softly drifting) ───────────────────────
        nebulae.forEachIndexed { i, nb ->
            val driftX  = (sin((time + i).toDouble()) * 0.018 * w).toFloat()
            val driftY  = (cos((time * 0.7 + i).toDouble()) * 0.014 * h).toFloat()
            val cx      = nb[0] * w + driftX
            val cy      = nb[1] * h + driftY
            val radius  = nb[2] * minOf(w, h)
            val hue     = nb[3]

            // Map hue → a dim nebula colour (purple / teal / rose)
            val nebulaColor = when {
                hue < 0.33f -> Color(0xFF1A0A2E).copy(alpha = 0.18f)  // deep violet
                hue < 0.66f -> Color(0xFF0A1E2E).copy(alpha = 0.15f)  // deep teal
                else        -> Color(0xFF2E0A18).copy(alpha = 0.15f)  // deep rose
            }
            drawCircle(color = nebulaColor, radius = radius, center = Offset(cx, cy))
        }

        // ── Stars (twinkle via sin wave per star) ─────────────────────────────
        stars.forEach { s ->
            val x           = s[0] * w
            val y           = s[1] * h
            val baseRadius  = s[2]
            ...
            // Each star's alpha oscillates independently between ~5 % and ~55 %
            val twinkle     = ((sin((time + phase).toDouble()) + 1.0) / 2.0).toFloat()
            val alpha       = 0.05f + twinkle * 0.50f
...
fun GameScreen(
    ...
    BoxWithConstraints(
        ...
        // Also slightly reduced in portrait (from 2.0 to 1.6) to make bottom row placement easier
        val currentOffsetFactor = if (isLandscape) 1.2f else 1.6f
        ...
        fun computeSnapCell(fx: Float, fy: Float, idx: Int): Pair<Int, Int> {
            ...
            // Added a small vertical bias (0.15f) to make it easier to snap into the row below
            val r = (fRow + 0.15f).roundToInt().coerceIn(0, BOARD_SIZE - blk.rows)
            val c = (fCol - blk.cols / 2f).roundToInt().coerceIn(0, BOARD_SIZE - blk.cols)
            return Pair(r, c)
        }
