...
fun GameBoard(
    ...
    BoxWithConstraints(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            val boardColor = if (isDarkTheme) BoardBackground else com.rpn.blockblaster.core.designsystem.BoardBackgroundLight
            val gridColor  = if (isDarkTheme) GridLine else com.rpn.blockblaster.core.designsystem.GridLineLight
...
fun DrawScope.draw3DBlockCell(rect: Rect, color: Color) {
    ...
    clipPath(basePath) {
        ...
        val innerGradient = Brush.radialGradient(
            ...
            center = Offset(innerRect.left + innerRect.width * 0.35f, innerRect.top + innerRect.height * 0.35f),
...
fun DrawScope.drawBlastingCell(rect: Rect, alpha: Float, color: Color?) {
    ...
    val extraRect = Rect(rect.left - amount, rect.top - amount, rect.right + amount, rect.bottom + amount)
    val glowBrush = Brush.radialGradient(
        colors = listOf(Color.White.copy(alpha = alpha * 0.9f), blastColor.copy(alpha = alpha * 0.5f), Color.Transparent),
        center = rect.center,
...
private fun DrawScope.drawGrid(cellSize: Float, boardSize: Int, gridColor: Color) {
    for (i in 1 until boardSize) {
        val x = i * cellSize; val y = i * cellSize
